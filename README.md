...
Download your Firebase Admin Service Account key and save it as firebase-key.json inside the backend folder.
...
npm install
Frontend Environment Variables: Create a .env.local file inside the frontend folder and add your Firebase client configuration:
...
docker build -t ahma-agong-monitor .
# Run the Docker container
docker run -p 8080:8080 ahma-agong-monitor
(Note: Make sure your environment variables and secret files are properly configured for your Docker environment or Google Cloud Run deployment).

🔑 Key Features
OCR MyKad Scanning: Quickly extracts patient Name and IC Number.
Voice Transcription: Records clinical notes hands-free.
AI Summarization: Uses Google Gemini to format messy voice notes into a structured medical assessment.
Automated PDF Reports: Generates professional PDF records with hospital branding.
Instant Notifications: Pushes notifications and PDFs directly to the suggested specialist doctor.
