...
const PatientMonitor = ({ id, p, onOpenProfile }) => {
  ...
  const isCritical = (p.heartRate > 125 || p.heartRate < 55) && !isDispatched;
  ...
  useEffect(() => {
    const calculatedSpo2 = p.heartRate > 125
  ...
  return (
    <div className="patient-card" style={{ backgroundColor: cardBgColor, borderColor: cardBorderColor }}>
      ...
      >
        {isDispatched ? 'View Status' : (isCritical ? 'Dispatch Ambulance' : 'View Details')}
      </button>
