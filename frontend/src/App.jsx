...
function App() {
  ...
  const handleSaveRecord = async () => {
    if (!patientName.trim() || !patientIc.trim()) return;
  ...
  const stopRecording = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:5001/stop", { method: "POST" });
      const data = await res.json();
  ...
  const isFormReady = patientName.trim() && patientIc.trim();
    ...
    <div className="layout">
      <header className="topbar">
        <div className="topbar-left">
          <div className="hospital-mark"><span className="cross-icon">✛</span></div>
          <div>
            <div className="hospital-name">Hospital Emergency Management System</div>
            <div className="hospital-sub">Patient Intake &amp; Assessment Module</div>
          </div>
        </div>
        <div className="topbar-right">
          ...
          <div className="status-badge"><span className="dot"></span> System Online</div>
        </div>
      </header>
      ...
      <footer className="footer">
        <span>Emergency Management System &mdash; Restricted Use</span>
      </footer>
    </div>
  );
}
